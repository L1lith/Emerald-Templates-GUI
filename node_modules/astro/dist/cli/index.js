...
async function cli(args) {
  ...
  const telemetry = new AstroTelemetry({ version: "1.0.0-beta.36" });
  if (cmd === "telemetry") {
    try {
  ...
  switch (cmd) {
    ...
    case "dev": {
      try {
        ...
        telemetry.record(event.eventCliSession({ astroVersion: "1.0.0-beta.36", cliCommand: "dev" }, userConfig, flags));
    ...
    case "build": {
      try {
        const { astroConfig, userConfig } = await openConfig({ cwd: root, flags, cmd });
        telemetry.record(event.eventCliSession({ astroVersion: "1.0.0-beta.36", cliCommand: "build" }, userConfig, flags));
    ...
    case "check": {
      const { astroConfig, userConfig } = await openConfig({ cwd: root, flags, cmd });
      telemetry.record(event.eventCliSession({ astroVersion: "1.0.0-beta.36", cliCommand: "check" }, userConfig, flags));
    ...
    case "preview": {
      try {
        const { astroConfig, userConfig } = await openConfig({ cwd: root, flags, cmd });
        telemetry.record(event.eventCliSession({ astroVersion: "1.0.0-beta.36", cliCommand: "preview" }, userConfig, flags));
